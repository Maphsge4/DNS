...
void Receive_from_Extern()
{
    ...
    if (length > -1)
    {
        ...
        memcpy(buf, &ID_Trans_Table[id_index].old_ID, sizeof(unsigned short));
...
void Receive_from_Local()
{
    ...
    if (length > 0)
    {
        ...
        string ip;
        map<string, string>::iterator i = local_table.find(url), j = cache.find(url);
        if (i == local_table.end() && j == cache.end()) /* Url not in local table and cache */
        {
        ...
        {
            if (i != local_table.end()) /* Local data */
            {
                ip = i->second; /* Get IP from local table */
                if (debug_level >= 1)
                    printf("Read from local data [Url:%s -> IP:%s]\n", url, ip.c_str());
            }
            else /* Cache */
            {
                ip = j->second;                 /* Get IP from cache */
                string url_str(url);            /* Create a Url string */
                cache_LRU_list.remove(url_str); /* Move the record to the top of the list*/
                cache_LRU_list.push_front(url_str);
                if (debug_level >= 1)
                {
            ...
            if (ip == "0.0.0.0")   /* Judge if the Url should be shielded */
                ...
                a = htons(0x0001); /* Server function : set the number of answer to 1 */
            ...
            unsigned long IP = (unsigned long)inet_addr(ip.c_str()); /* Actually data is IP */
...
void Process_Parameters(int argc, char *argv[])
{
    ...
    {
        if (argv[1][1] == 'd')
            debug_level++; /* Debug level add to 1 */
        if (argv[1][2] == 'd')
            debug_level++; /* Debug level add to 2 */
        if (argc > 2)
        {
